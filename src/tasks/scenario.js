...
// `deadEnd` names an item whose `next` pointer the server cuts (follow's unanswerable variant).
export async function createScenario({ low = 3, size = 20, seed = null, stress = null, injection = "write", strict = false, deadEnd = null } = {}) {
  return api("POST", "/api/scenarios", { low, size, ...(seed !== null ? { seed } : {}), ...(stress ? { stress, injection } : {}), ...(strict ? { strict: true } : {}), ...(deadEnd ? { deadEnd } : {}) });
}
