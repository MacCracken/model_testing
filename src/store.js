...
create table if not exists trials (
  ...
  rounds integer, finish_reason text, started_at text, canon text, skill text, base_client text, agents text, delegations integer,
...
const LATER_COLUMNS = {
  ...
  trials: { canon: "text", skill: "text", base_client: "text", agents: "text", delegations: "integer" },
    ...
    if (run.status !== "running") {
      const ins = d.prepare(`insert into trials
        (run_id, idx, task, mode, client, model, harness, trial_index, correct, reason, error, tool_calls, tool_use_ok, tool_use_reason,
         schema_valid, judge_score, judge_reason, latency_ms, ttft_ms, ttfa_ms, prompt_tokens, completion_tokens, total_tokens, rounds, finish_reason, started_at, canon, skill, base_client, agents, delegations)
        values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);
      (run.rows ?? []).forEach((r, i) => ins.run(
        ...
        r.skill?.how ?? null, r.baseClient ?? null, r.agents?.how ?? null, r.agents ? num(r.agents.delegations) ?? 0 : null,
      ));
