  ...
  try {
    ...
    if (run.status !== "running") {
      const ins = d.prepare(`insert into trials
        ...
        values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);
      const lineageOfRow = (r) => run.config?.lineage?.[r.client] ?? run.config?.lineage?.[String(r.client).replace(/@(skill|agents|stress|constraints|format|effort|confidence|abstain)(:[a-z]+)?$/, "")] ?? null;
