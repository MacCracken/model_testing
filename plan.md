...
the last built item of the original roadmap landed.

## Start here (handoff, 2026-09-14)
...
| Calibration & abstention | HELM calibration; answer-or-abstain splits | `@confidence` (Brier, ECE, gap), `@abstain` on three families, `norelevant` / `nearmiss` | unanswerable variants for the tool and extraction families ([48]) | low |
| Robustness / consistency | HELM perturbations; paraphrase suites | agreement per instance, variance by setting, `@perturb` on four families with consistency | perturbations for the tool families (item names, endpoint wording) ([48]) | low |
...
- abstention and perturbation for the tool and extraction families — a scenario with the asked-for
  item missing, an invoice without the asked-for field, item names and endpoint wording rewritten
  (small each; the hooks exist);
