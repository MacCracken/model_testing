...
- **Run it.** `npm test` (320 tests; no model or server needed), then `node src/cli.js serve` for
...
- **Next**: the open harness work in Tier 12 ([42]–[44], [47]); [40]'s log import when there
  are Inspect or lm-eval logs to test it against; the rest of [48] (generator and constraint
...
| Tests | 320, none needing a model; the webserver runs in-process |
...
1. **Order for the next month.** Recommendation: [47] (variance across settings — cheap, and
   the effort knob now makes "thinking on versus off" one of the settings), then [42]–[44] as
   the arms' channels allow; the rest of [48] as tiers saturate; [40]'s log import when logs exist.
